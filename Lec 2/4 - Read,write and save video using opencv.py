...
cap = cv2.VideoCapture(0,cv2.CAP_DSHOW)   #Here parameter 0 is a path of any video use for webcam and 1 if using other webcam
...
while(cap.isOpened()):
    ...
    if ret==True:
        ...
        cv2.imshow("Gray Frame",gray)
        cv2.imshow('Colorframe',frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):   #press to exit // 1 as multiple frames if image than zero
            break
    else:
        break
